...
(() => {
  ...
  // gallery (additional photos) — support both old/new ids
  const admImgsList = document.getElementById("admImgsList") || document.getElementById("admGalleryList");
  const admImgsDrop = document.getElementById("admImgsDrop") || document.getElementById("admDropMore");
  const admImgsPick = document.getElementById("admImgsPick") || document.getElementById("admPickMore");
  const admImgsFile = document.getElementById("admImgsFile") || document.getElementById("admFilesMore");
  ...
  function openAdminModal(product = null) {
    ...
    // gallery preload from product.imgs (exclude main; we keep "extra only" here)
    const allImgs = normalizeImgs(product?.imgs);
    const main = (product?.img || allImgs[0] || "").trim();
    galleryItems = uniqUrls(allImgs.filter(u => u && u !== main)).map(u => ({ url: u }));
  ...
  // gallery dropzone
  admImgsPick?.addEventListener("click", () => admImgsFile?.click());
  ...
  function addGalleryFiles(fileList) {
    ...
    files.forEach((f) => {
      ...
      galleryItems.push({ file: f, url });
    });
    renderGalleryList();
  }

  admImgsFile?.addEventListener("change", () => {
    addGalleryFiles(admImgsFile.files);
    // reset so same file can be re-added
    try { admImgsFile.value = ""; } catch {}
  });

  ["dragenter", "dragover"].forEach((ev) => {
    admImgsDrop?.addEventListener(ev, (e) => {
      e.preventDefault();
      admImgsDrop.classList.add("is-drag");
    });
  });
  ["dragleave", "drop"].forEach((ev) => {
    admImgsDrop?.addEventListener(ev, (e) => {
      e.preventDefault();
      admImgsDrop.classList.remove("is-drag");
    });
  });

  admImgsDrop?.addEventListener("drop", (e) => {
  ...
  function renderGalleryList() {
    ...
    const main = (admImg?.value || "").trim();

    const items = galleryItems.map((it, idx) => {
      const u = String(it.url || "").trim();
      return `
        <div class="adm-imgs-item" data-idx="${idx}">
          <img src="${escHtml(u)}" alt="">
          <div class="adm-imgs-actions">
            <button type="button" class="adm-imgs-btn" data-set-main="${idx}" title="Зробити головним">★</button>
            <button type="button" class="adm-imgs-btn" data-del="${idx}" title="Видалити">✕</button>
          </div>
        </div>
      `;
    }).join("");
    ...
    // show hint if none
    const hint = !galleryItems.length ? `<div class="muted" style="margin-top:8px">Додай фото — вони підуть у модалку як галерея.</div>` : "";
    const mainHint = main ? `<div class="muted" style="margin-top:8px">Головне фото показується першим.</div>` : "";
    admImgsList.innerHTML = items + hint + mainHint;

    admImgsList.querySelectorAll("[data-del]").forEach((btn) => {
    ...
    admImgsList.querySelectorAll("[data-set-main]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const i = parseInt(btn.getAttribute("data-set-main"), 10);
        if (!Number.isFinite(i)) return;
        const it = galleryItems[i];
        if (!it?.url) return;

        // move old main into gallery (if it exists and not already in gallery)
        const oldMain = (admImg?.value || "").trim();
        admImg.value = it.url;
        selectedFile = null; // main is now URL
        setPreview(it.url);

        // remove chosen from gallery
        galleryItems.splice(i, 1);

        // add previous main to gallery if needed
        if (oldMain && !galleryItems.some(x => String(x.url).trim() === oldMain)) {
          galleryItems.unshift({ url: oldMain });
        }
  ...
  async function uploadGalleryFiles(productId) {
    // upload only items that have file
    for (const it of galleryItems) {
      if (!it.file) continue;
      const url = await uploadToStorage(it.file, productId);
      it.url = url;
      delete it.file;
    }
    galleryItems = uniqUrls(galleryItems.map(x => x.url)).map(u => ({ url: u }));
    renderGalleryList();
  }
  ...
  async function editProduct(id) {
    const { data, error } = await sb
      .from("products")
      .select(`
        id, title, price, stock, img, imgs, desc, is_popular, is_active, category, category_id
      `)
      .eq("id", id)
      .single();
  ...
  admSave?.addEventListener("click", async () => {
    ...
    const stock = admStock ? Math.max(0, safeInt(admStock.value)) : undefined;
    ...
    try {
      // if main file selected -> upload as main
      if (selectedFile) {
        img = await uploadToStorage(selectedFile, productId);
        admImg.value = img;
        selectedFile = null;
      }

      // upload gallery new files (if any)
      await uploadGalleryFiles(productId);
    ...
    // FINAL imgs array: [main, ...gallery]
    const extra = galleryItems.map(x => x.url).filter(Boolean);
    const imgs = uniqUrls([img, ...extra]);
    ...
    if (editingId) {
      ...
      res = await sb.from("products").insert([
        { id: productId, ...payload, created_at: new Date().toISOString() }
      ]);
    }
